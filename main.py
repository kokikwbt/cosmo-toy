...
if args.minmax_scale:
    print("MinMaxScale")
    # tts = cosmo.utils.MinMaxScaler().fit_transform(tts)
...
if args.grid_search:
    ...
    # save results
    cosmo.utils.plot_grid_search_result(
...
config = vars(args)
cosmo.utils.saveas_json(args.outdir + "/config.json", config)
...
for t in range(ts, ed - wd - ls - dt, dt):
    ...
    rec_df = cosmo.utils.pred2df(rec_, window_index=t)
    pred_df = cosmo.utils.pred2df(pred, window_index=t+wd)
    ...
    rec_intr_df = cosmo.utils.pred2df(rec_intr, window_index=t)
    pred_intr_df = cosmo.utils.pred2df(pred_intr, window_index=t+wd)  # window_id: end point of current window
    rec_intr_df_list.append(rec_intr_df)
    pred_intr_df_list.append(pred_intr_df)

    # save latent sequences
    latent_seq_df = cosmo.utils.seq2df(latent_seq, window_index=t)
